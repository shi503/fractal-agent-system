...
You are bootstrapping a new FRACTAL epic session. The argument is the blueprint filename (in `.fractal/`).

## Steps

1. **Verify infrastructure:**
   ```bash
   ls .fractal/router.py
   ls .fractal/$ARGUMENTS
   ```
...
4. **Print session brief:**
   ...
   - Recommended next step: "Start with workstream X using the Feature Lead rules"

5. **Remind:**
   Before starting each workstream, run:
   ```bash
   python3 .fractal/router.py update <FeatureLeadName> IN_PROGRESS
   ```
